...
{
	NodeInfo& node;
	std::unordered_map<eCommandClass, std::unique_ptr<CCHandler>> handlers;

public:
	CCHandlerFactory(NodeInfo& n) : node(n) {}

	template<typename T>
	void AddHandler()
	{
		auto ccId = T::CC;
		handlers[ccId] = std::unique_ptr<CCHandler>(new T(node));
	}

	CCHandler* GetHandler(eCommandClass ccId)
	{
...
class CC_Version : public CCHandler
{
public:
